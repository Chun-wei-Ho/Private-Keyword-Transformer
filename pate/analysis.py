...
tf.flags.DEFINE_bool("input_is_counts", False, "False if labels or teacher_preds, True if counts")
tf.flags.DEFINE_bool("input_is_teacher_preds", False, "True if input is teacher_preds")
tf.flags.DEFINE_integer("n_chunks", 1, help="Number of chunks in testing set")
tf.flags.DEFINE_integer("chunk_id", 0, help="Chunk id in testing set, starting from 0")
...
def main(unused_argv):
  ...
  if FLAGS.input_is_teacher_preds:
    input_mat = input_mat[:, FLAGS.chunk_id::FLAGS.n_chunks, :]
    _, _, input_mat \
      = aggregation.noisy_max(input_mat, 0, return_clean_votes=True)
    FLAGS.input_is_counts = True
  ...
  print("..add noise ~ " + str(ss_scale)) # ss_scale = 1/r
  print("... times " + str(min(total_ss_nm / l_list)))
